...
;// CONCATENATED MODULE: ./js/entry/popup.js
...
(async function () {
  util_instance.create_window_api({
    ...
    fetch_json: ([url, data]) => {
      return util_instance.fetch_json(url, data);
    },
    get_vimeo_thumb_url: async (vimeo_id) => {
      var r = await fetch(`https://player.vimeo.com/video/${vimeo_id}`);
      var text = await r.text();
      let match = text.match(/https\:\/\/i\.vimeocdn\.com\/video\/[^_]+?_640/);
      return match[0];
      var r = await fetch(`https://vimeo.com/${vimeo_id}`);
      var text = await r.text();
      var parser = new DOMParser();
      var doc = parser.parseFromString(text, "text/html");
      var url1 = doc.querySelector(`[property="og:image"]`).content;
      var url2 = new URL(url1);
      var url3 = url2.searchParams.get("src0");
      var url4 = url3.replace("1280x720", "640");
      return url4;
    },
  });
  // inject iframe
