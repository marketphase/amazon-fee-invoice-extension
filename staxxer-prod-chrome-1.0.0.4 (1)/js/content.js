...
;// CONCATENATED MODULE: ./js/entry/content.js
...
class Sidebar {
  inject() {
    ...
    iframe.name = JSON.stringify({ context: "sidebar", url: location.href, hostname: location.hostname });
    this.iframe_wrap = util_instance.create_iframe_wrap(iframe);
    iframe.src = util_instance.get_iframe_src();
    console.log("iframe", iframe);
    document.querySelector("#chromane_iframe_container").appendChild(iframe);
    document.querySelector("#chromane_iframe_toggle_button").addEventListener("click", function () {
      console.log("click");
      document.querySelector("#chromane_iframe_container").classList.toggle("active");
    });
  }
}
...
(function () {
  ...
  util_instance.inject_css();
  sidebar.inject();
  let iframe_wrap = null;
  sidebar.iframe_wrap.then((wrap) => {
    iframe_wrap = wrap;
    let html = document.querySelector("#sendFormBean").outerHTML;
    ...
    wrap.exec("set", ["buttons", data.buttons]);
  });
  util_instance.create_window_api({
    get_pdf_blob: async (data) => {
      let body = {
        endDate: data.enddate,
        fileType: data.filetype,
        filterName: data.filtername,
        vatInvoiceNumber: data.invoice,
        payeeRegistrationNumber: data.payeeregistrationnumber,
        documentVersionId: data.value,
      };
      let json = await util_instance.fetch_json("https://sellercentral.amazon.de/tax/view-seller-fee-invoice-execute", {
        headers: {
          accept: "*/*",
      ...
      let pdf_url = json.url;
      let pdf_r = await fetch(pdf_url);
      let pdf_blob = await pdf_r.blob();
      return pdf_blob;
    },
  });
})();
