...
const parseInvoicesFromPage = (): Invoice[] => {
...
export default function App() {
  ...
  // Format date for display (e.g., "01/12/2025")
  ...
  // Get unique marketplaces from invoices
  const marketplaces = [...new Set(invoices.map(inv => inv.marketplace).filter(Boolean))].sort();
        ...
        <div style={{ padding: '16px 20px', borderBottom: '1px solid #e5e5e5' }}>
          <div style={{ marginBottom: '12px' }}>
            <label
              ...
              style={{ fontSize: '12px', color: '#666666', display: 'block', marginBottom: '6px' }}>
              Marketplace
            </label>
            <select
        ...
        <div style={{ padding: '16px 20px', borderBottom: '1px solid #e5e5e5' }}>
          ...
          <button
            ...
            style={{
              width: '100%',
              padding: '10px 16px',
              backgroundColor: filteredInvoices.length > 0 ? '#5547CF' : '#e5e5e5',
                ...
                <div>
                  ...
                  <div style={{ fontSize: '12px', color: '#9a7d0a', lineHeight: 1.5 }}>
                    {downloadResult.success > 0 && (
                      <span>
                        {downloadResult.success} invoice{downloadResult.success !== 1 ? 's' : ''} downloaded.{' '}
                      </span>
                    )}
                    {downloadResult.failed.length} failed due to Amazon server errors:{' '}
                    {downloadResult.failed.join(', ')}
                  </div>
                  <button
          ...
          {filteredInvoices.length === 0 ? (
            ...
            filteredInvoices.map((invoice, index) => (
              <div
                ...
                <div style={{ fontSize: '12px', color: '#666666', marginBottom: '4px' }}>{invoice.invoiceType}</div>
                <div style={{ fontSize: '11px', color: '#999999' }}>
                  {formatDate(invoice.date)}
                  {invoice.endDate ? ` to ${formatDate(invoice.endDate)}` : ''}
                </div>
              </div>
            ))
          )}
